...
parser.add_argument("-i", "--input_dir", help = "change the input directory", default = "./input_data") 
parser.add_argument("-o", "--output_dir", help = "change the output directory", default = "./input")
parser.add_argument("-m", "--mode", help = "choose between compressed and decompressed", \
    choices = ["compressed", "decompressed", "both"], default = "both")
...
for filepath in glob.iglob(args.input_dir + "/*.tar.gz"):
    ...
    current_label = filepath[13:filepath.find("_filtered_gene_bc_matrices.tar.gz")]
