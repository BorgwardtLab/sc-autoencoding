...
for aename in ${autoencoders[@]}; do
	...
	python ../3_Autoencoder/ex_bca_noscaling/bca_autoencoder_${aename}.py --mode complete --loss poisson --activation relu --optimizer RMSprop --input_dir $preprocessed_ctdata --output_dir "${outdir}bca_data/${aename}/" --outputplot_dir "${outdir}bca_data/${aename}/"  |& tee -a $logfile
...
printf "\nBCA experiment noscaling took %d minutes\n" `echo "($end-$start)/60" | bc` |& tee -a $logfile
printf "\n################### " &>> $logfile
